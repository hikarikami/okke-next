<script lang="ts">
	...
	const invoiceTheme = $derived(data.invoiceTheme);
	const brandColour = $derived(data.brandColour);

	// --- Form state (pre-filled from existing invoice) ---
	let invoiceNumber = $state(data.invoice.number);
	let issueDate = $state(data.invoice.issueDate);
	let dueDate = $state(data.invoice.dueDate);

	let fromName = $state(data.invoice.from.name ?? '');
	let fromAddress = $state(data.invoice.from.address ?? '');
	let fromEmail = $state(data.invoice.from.email ?? '');
	let fromPhone = $state(data.invoice.from.phone ?? '');

	let toName = $state(data.invoice.billTo.name ?? '');
	let toAddress = $state(data.invoice.billTo.address ?? '');
	let toEmail = $state(data.invoice.billTo.email ?? '');
	...
	let taxEnabled = $state(data.invoice.taxEnabled);
	let taxRate = $state(data.invoice.taxRate);
	let notes = $state(data.invoice.notes ?? '');
	let paymentTerms = $state(data.invoice.paymentTerms ?? '');
...
<div class="flex h-full flex-col">
	...
	>
		<div class="flex items-center gap-3">
			...
			<div>
				<h1 class="text-base font-semibold text-gray-900 dark:text-gray-100">Edit Invoice</h1>
					...
					<div>
						...
						{#snippet contactFooter()}
							{#if contactInput.trim()}
								<div class="border-t border-gray-100 p-1 dark:border-gray-800">
									<form
										method="POST"
										action="/invoices/new?/createContact"
						...
						>
							<div class="relative">
								...
								{:else if !contactInputFocused && toName && !selectedContactId}
									<span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-900 dark:text-gray-100">
										{toName}
									</span>
								{/if}
								<ComboboxInput
									...
									placeholder={selectedContact || toName ? '' : 'Search or create a contact...'}
									class={(selectedContact || (toName && !contactInputFocused)) ? 'text-transparent dark:text-transparent' : ''}
									onclear={(selectedContactId || toName) ? () => {
										selectedContactId = '';
										toName = '';
										toEmail = '';
										contactInput = '';
